...
public class CardBartok : Card {
	...
	// Fields tostore info he card will use to move and rotate
	...
	// his overload of MoveTo doesn't require a rotation argument
	...
	void Update(){
		...
		case CBState.to:
			// Get u from the current time andduration
			// u ranes from 0 to 1 (usually)
			...
			if (u<0){ // If u<0, then we shouldn't move yet
				...
				// Reset timeStart to 0 so it ts overwritten next time
				timeStart = 0;
				
				if (reportFinishTo != null){ // If there's a callback object
					// ...then use SendMessage to call the CBCallback method
					// with this as the parameter
					reportFinishTo.SendMessage("CBCallback", this);
					// After calling SenMessage(), reportFinshTo must be set
